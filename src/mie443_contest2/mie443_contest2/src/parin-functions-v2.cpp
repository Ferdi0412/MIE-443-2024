...
static std::vector<cv::Ptr<cv::Feature2D>> feature_detectors;
...
static unsigned int default_min_hessian = 400;
...
/**
 * Toggles whether or not to display the images for each time match_function is run
*/
void draw_all_matches( bool draw_all_matches = true ) {
    draw_matches = draw_all_matches;
}

/**
 * Sets how many good_matches must be found for a given template to be "valid"
*/
...
/**
 * set_default_min_hessians will set a single min_hessian for all feature_detectors IF:
 * 1. Run before initialize_feature_detector AND
 * 2. No min_hessians value is given in initialize_feature_detector
*/
void set_default_min_hessians( unsigned int min_hessian ) {
    default_min_hessian = min_hessian;
}

/**
 * initialize_feature_detector must be run before match_function at least once
*/
void initialize_feature_detector( const std::vector<cv::Mat>& box_templates, std::vector<unsigned int> min_hessians = {400, 400, 1400} ) {
    std::vector<unsigned int> min_hessians_used = (min_hessians.size() == box_templates.size()) ? min_hessians : std::vector<unsigned int>(box_templates.size(), default_min_hessian);
    ...
    for ( size_t i = 0; i < box_templates.size(); i++ ) {
        feature_detectors.push_back( SURF::create( min_hessians_used[i]) );
        ...
        feature_detectors[i]->detectAndCompute( img_template, cv::Mat(), keypoints_this_template, descriptors_this_template );
...
int match_function( const cv::Mat& img, const std::vector<cv::Mat>& box_templates ) {
    ...
    */
    ...
    for ( size_t i = 0; i < box_templates.size(); i++ ) {
        // Update for scene using specific template feature detector
        feature_detectors[i]->detectAndCompute( img_scene, cv::noArray(), keypoints_scene, descriptors_scene );
